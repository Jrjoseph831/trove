...
export class TroveStack extends Stack {
  constructor(scope: Construct, id: string, props?: TroveStackProps) {
    ...
    // ── S3 assets bucket (Studio logos, banners, product images) ───────────
    // Objects are publicly readable via bucket policy; uploads go through
    // presigned PUT URLs (5-min expiry) generated by the Upload Lambda.
    const assets = new s3.Bucket(this, "Assets", {
      bucketName: isProd ? "trove-assets" : undefined,
      removalPolicy: retain,
      blockPublicAccess: new s3.BlockPublicAccess({
        blockPublicAcls: true,
        ignorePublicAcls: true,
        blockPublicPolicy: false,    // allow bucket policy public read
    ...
    // Upload Lambda — issues presigned PUT URLs; client uploads directly to S3.
    const upload = fn("Upload", "upload.ts", Duration.seconds(10));
    assets.grantPut(upload);
    upload.addEnvironment("ASSETS_BUCKET", assets.bucketName);
    api.addRoutes({
      path: "/studio/upload",
      methods: [HttpMethod.POST],
      integration: new HttpLambdaIntegration("UploadIntegration", upload),
      authorizer,
    });
