...
- **BETA** is significantly ahead of prod and is the active branch. Not merged (awaiting OK). Since the last prod merge:
  ...
  - **Deferred, larger, separate projects** (explicitly NOT started): AI-to-AI production/trading (AI companies currently only draw from an abstract shared pool — see "AI economy" above — they don't hold real inventory or sell to each other), and AI releasing brand-new products over time (the catalog is a static seed today). Both are bigger systems projects than the above; sequence to be decided.
