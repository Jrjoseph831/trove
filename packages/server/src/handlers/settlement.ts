/**
 * Settlement Lambda — the heartbeat of the shared MARKET.
 *
 ...
 * runs) to advance prices/news/sectors, and writes it back. The server owns the
 * clock; no client input can advance it. Idempotent and self-healing: it catches
 * up to the wall-clock cycle if a fire was missed, capped so a long outage can't
 * roll a hundred stories.
 *
 * Player FACTORY production is NOT here — it runs on a separate fast clock (the
 * Production Lambda, every few minutes) so the floor feels alive while news and
 * prices turn on this slow 6h beat. See handlers/production.ts.
 */
